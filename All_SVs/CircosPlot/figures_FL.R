...
#hg38.genes = hg38.genes[!(hg38.genes$gene_type %in% c("lincRNA","intergenic")),]
...
######Figure1
datafile <- read.table("FL_P26789.sv.txt",header = T)
...
genes <- unique(c(as.character(annoteSVwithGene(sv1_start,hg38g)$gene_name),as.character(annoteSVwithGene(sv2_start,hg38g)$gene_name),
                  as.character(annoteSVwithGene(sv1_end,hg38g)$gene_name),as.character(annoteSVwithGene(sv2_end,hg38g)$gene_name),as.character(datafile$GeneB)))
...
pdf(file = paste0("./Figure1C_FL.pdf"),width=10,height=8, onefile = T)
