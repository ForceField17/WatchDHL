...
plot.table2 <-plot.table[which(  (plot.table$Tissue!="B.FL.a" & (plot.table$Tissue=="C.DHL" | plot.table$Fusion!="MYC" ))),] 
...
the<-compare_means(fraction ~ ID,  data = plot.table2,paired = F )
the <- the[c(1,4,6),]
...
ggsave(file="AF_change.pdf", plot=figure_2,bg = 'white', width =20, height = 16, units = 'cm', dpi = 600)
