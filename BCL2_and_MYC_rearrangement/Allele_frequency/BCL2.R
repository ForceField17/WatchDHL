...
NAN_plot <- NAN_plot + geom_violin(data=plot.table2,aes(x=Fusion,y=as.numeric(fraction),fill=Fusion),width=0.6,alpha=0.4,size=0.4) +
  geom_boxplot(data=plot.table2,aes(x=Fusion,y=as.numeric(fraction)),width=0.1,alpha=0.5,size=0.6)+
  geom_path(data=plot.table2,aes(x=Fusion,y=fraction,group=Case),color="grey40",size=0.3) +
 ...
 # geom_path(data=plot.table4,aes(x=Fusion,y=fraction,group=Case),color="black",size=0.3) +
  ...
  geom_text(aes(x=Fusion,y=as.numeric(fraction),label=Case),data=plot.table2,size=2.2) 
...
ggsave(file="AF_BCL2_newxx.pdf", plot=figure_2,bg = 'white', width =14, height = 12, units = 'cm', dpi = 600)
