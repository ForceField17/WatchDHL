...
samples <- samples[which(samples$Cell!="CL1" & samples$Line!="Repair"),]
################
...
ggsave(file="./Fig_OE_vs_WT/PCA.pdf", plot=figure_1,bg = 'white', width = 14, height = 10, units = 'cm', dpi = 600)
########
...
final_data <- data.frame(geneName,rowMean,rowSD,AnovaP_genetic,FDR_genetic,Rank_genetic,AnovaP_OPP,FDR_OPP,Rank_OPP,Rank_genetic_GSEA,Rank_OPP_GSEA )
write.csv(final_data , file = "./Fig_OE_vs_WT/ANOVA_all_genesEXP.csv",sep = "\t",quote = F,row.names = F)
...
ggsave(file="./Fig_OE_vs_WT/TwoWay_ANOVA.pdf", plot=figure_4,bg = 'white', width = 10, height = 7, units = 'cm', dpi = 600)
...
TheTable <- fgseaResTidy[which(fgseaResTidy$pvalue <= 0.01 & fgseaResTidy$p.adjust <= 0.1),]
...
ggsave(file="./Fig_OE_vs_WT/KEGG_GSEA.pdf", plot=figure_2,bg = 'white', width = 16, height = 16, units = 'cm', dpi = 600)
...
for(x in c(1:12)){
  ...
  ggsave(file=paste0("./Fig_OE_vs_WT/KEGG/kegg_pathways_",x,".pdf"), plot=figure,bg = 'white', width = 9, height = 7, units = 'cm', dpi = 600)
}
