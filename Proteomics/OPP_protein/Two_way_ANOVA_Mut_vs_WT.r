...
samples <- samples[which(samples$Cell!="CL1" & samples$Line!="OE"),]
################
...
xxx <- counts[,c(4:6,10:12)]
yyy <- counts[,c(4:6)]
zzz <- counts[,c(10:12)]
keep <- (rowSums(yyy == min(counts)) <= 0 | rowSums(zzz == min(counts)) <= 0)
counts <- counts[keep,]
nrow(counts)
...
ggsave(file="./Fig_Mut_vs_WT/PCA.pdf", plot=figure_1,bg = 'white', width = 14, height = 10, units = 'cm', dpi = 600)
########
...
final_data <- data.frame(geneName,AnovaP_genetic,FDR_genetic,Rank_genetic,AnovaP_OPP,FDR_OPP,Rank_OPP,Rank_genetic_GSEA,Rank_OPP_GSEA )
write.csv(final_data , file = "./Fig_Mut_vs_WT/ANOVA_all_genesEXP.csv",sep = "\t",quote = F,row.names = F)
...
mySub2 <- mySub2 + xlab("Log10(FDR of genetic factor)") + ylab("Log10(FDR of OPP label factor)") +
  scale_y_continuous(expand=c(0,0),limits=c(-12,12),breaks=seq(-20,20,5)) + 
  scale_x_continuous(expand=c(0,0),limits=c(-9,9),breaks=seq(-8,8,4))
...
ggsave(file="./Fig_Mut_vs_WT/TwoWay_ANOVA.pdf", plot=figure_4,bg = 'white', width = 10, height = 7, units = 'cm', dpi = 600)
...
TheTable <- fgseaResTidy[which(fgseaResTidy$pvalue <= 0.01 & fgseaResTidy$p.adjust <= 0.01),]
...
plot2 <- plot2 + geom_bar(data=TheTable, aes(x=reorder(ID,-ADP), y=ADP,fill = adj),alpha=0.7,stat='identity',width=0.5) + 
  #geom_point(data=TheTable, aes(x=reorder(ID,-ADP), y=ADP,fill = adj,size=-log10(p.adjust)),shape=21,alpha=1) +
  geom_hline(yintercept = 0,linetype=1,color="black")+ scale_y_continuous(expand=c(0,0),limits=c(-2.8,2.8),breaks=seq(-4,4,1),position = "left") +
  ...
  #scale_size(name   = "FDR", range = c(min(-log10(TheTable$p.adjust)),max(-log10(TheTable$p.adjust))) ,  breaks = c(1,-log10(0.05),2,3,4,5,6),labels = expression( 0.1, 0.05,10^-2,10^-3,10^-4,10^-5,10^-6),guide = guide_legend(nrow=5)) +
...
ggsave(file="./Fig_Mut_vs_WT/KEGG_GSEA.pdf", plot=figure_2,bg = 'white', width = 16, height = 16, units = 'cm', dpi = 600)
...
for(x in c(1:12)){
  ...
  ggsave(file=paste0("./Fig_Mut_vs_WT/KEGG/kegg_pathways_",x,".pdf"), plot=figure,bg = 'white', width = 9, height = 7, units = 'cm', dpi = 600)
}
