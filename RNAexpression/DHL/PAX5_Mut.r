...
Sample$Mutation <- "WT"
Sample$Mutation[which(( Sample$PAX5.noncoding == "Mut") | Sample$ZCCHC7 == "Gain")] <- "Mut"
...
NAN_plot <- NAN_plot + 
  geom_jitter(data=Sample,aes(x=reorder(Mutation,-as.integer(as.factor(Mutation))),y=Zscore_ZCCHC7,shape=ZCCHC7,color=PAX5.noncoding),size=2,width=0.12,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Mutation,-as.integer(as.factor(Mutation))),y=Zscore_ZCCHC7),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
...
NAN_plot <- NAN_plot + scale_color_manual(name=NULL,values = c(Mut="#e31a1c",WT="#8dd3c7",NotAvailable="grey"))+
  scale_shape_manual(name=NULL,values = c(WT=16,Gain=17))
the<-compare_means(p.adjust.method = "none",Zscore_ZCCHC7 ~ Mutation,  data = Sample,paired = F )
...
ggsave(file="ZCCHC7_mut_pax5TSS2.pdf", plot=figure_2,bg = 'white', width =12, height = 10, units = 'cm', dpi = 600)
...
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_jitter(data=Sample,aes(x=reorder(Mutation,-as.integer(as.factor(Mutation))),y=Zscore_PAX5,shape=ZCCHC7,color=PAX5.noncoding),size=2,width=0.12,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Mutation,-as.integer(as.factor(Mutation))),y=Zscore_PAX5),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
NAN_plot <-NAN_plot + theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(2,2,2,2),'lines'),
                            plot.title=element_text(size=34,vjust=0.5,hjust=0.5,face='bold.italic',color='transparent'),text=element_text(size=14,face='bold'),
                            legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.6,'cm'),legend.position='right',legend.text=element_text(size=14,hjust=0,face='bold'),
                            axis.text.x=element_text(size=12,face='bold',color='black'),axis.text.y=element_text(size=12,face='bold',color='black'),
...
NAN_plot <- NAN_plot + scale_color_manual(name=NULL,values = c(Mut="#e31a1c",WT="#8dd3c7",NotAvailable="grey"))+
  scale_shape_manual(name=NULL,values = c(WT=16,Gain=17))
the<-compare_means(p.adjust.method = "none",Zscore_PAX5 ~ Mutation,  data = Sample,paired = F )
#NAN_plot<- NAN_plot +stat_compare_means(comparisons = my_comparisons)
NAN_plot <- NAN_plot +stat_pvalue_manual(
  the, label = NULL, size = 3,linetype = 1,
  y.position = c( 2.9)
...
ggsave(file="PAX5_mut_pax5TSS2.pdf", plot=figure_2,bg = 'white', width =12, height = 10, units = 'cm', dpi = 600)
