...
selected <- which(samples$Histology!="x" & samples$tissue!="Tumor"  & samples$Type=="DHL" & samples$case!="RG047" & samples$Pos37026315GC!="NotAvailable")
...
NAN_plot <- NAN_plot + 
  geom_jitter(data=Sample,aes(x=reorder(Pos37026315GC,-as.integer(as.factor(Pos37026315GC))),y=Zscore_ZCCHC7,color=ZCCHC7),size=1.5,width=0.12,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Pos37026315GC,-as.integer(as.factor(Pos37026315GC))),y=Zscore_ZCCHC7),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
...
the<-compare_means(p.adjust.method = "none",Zscore_ZCCHC7 ~ Pos37026315GC,  data = Sample,paired = F )
...
ggsave(file="ZCCHC7_mut_pax5TSS2_1516.pdf", plot=figure_2,bg = 'white', width =10, height = 10, units = 'cm', dpi = 600)


##################3333
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_jitter(data=Sample,aes(x=reorder(Pos37026315GC,-as.integer(as.factor(Pos37026315GC))),y=Zscore_PAX5,color=ZCCHC7),size=1.5,width=0.12,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Pos37026315GC,-as.integer(as.factor(Pos37026315GC))),y=Zscore_PAX5),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
NAN_plot <-NAN_plot + theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(2,2,2,2),'lines'),
                            plot.title=element_text(size=34,vjust=0.5,hjust=0.5,face='bold.italic',color='transparent'),text=element_text(size=14,face='bold'),
                            legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.6,'cm'),legend.position='right',legend.text=element_text(size=14,hjust=0,face='bold'),
                            axis.text.x=element_text(size=12,face='bold',color='black'),axis.text.y=element_text(size=12,face='bold',color='black'),
...
NAN_plot <- NAN_plot + scale_fill_manual(name=NULL,values = c("#bababa","transparent"))
NAN_plot <- NAN_plot + scale_color_manual(name=NULL,values = c("#e31a1c","#8dd3c7"))
the<-compare_means(p.adjust.method = "none",Zscore_PAX5 ~ Pos37026315GC,  data = Sample,paired = F )
#NAN_plot<- NAN_plot +stat_compare_means(comparisons = my_comparisons)
NAN_plot <- NAN_plot +stat_pvalue_manual(
  the, label = NULL, size = 3,linetype = 1,
  y.position = c( 2.9)
...
ggsave(file="PAX5_mut_pax5TSS2_1516.pdf", plot=figure_2,bg = 'white', width =10, height = 10, units = 'cm', dpi = 600)
