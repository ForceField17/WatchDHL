...
selected <- which(samples$Site=="DLBCL" & samples$Type!="NotAvailable" & samples$tissue=="CellLine" )# & samples$Class=="Primary")
...
ggsave(file=paste0("./Linear_DHL_",gene[1],"_",gene[2],".pdf"), plot=plotxxx,bg = 'white', width = 17, height = 14, units = 'cm', dpi = 600)
...
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitter(width = 0.16),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_ZCCHC7,color=ZCCHC7),size=1.5,width=0.12,alpha=1)+
...
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(-3,3.2),breaks = seq(-10,10,1)) 
...
ggsave(file="ZCCHC7_DHL.pdf", plot=figure_2,bg = 'white', width =10, height = 10, units = 'cm', dpi = 600)


##################3333
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitter(width = 0.16),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_PAX5,color=ZCCHC7),size=1.5,width=0.12,alpha=1)+
...
ggsave(file="PAX5_DHL.pdf", plot=figure_2,bg = 'white', width =10, height = 10, units = 'cm', dpi = 600)


##################3333
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitter(width = 0.16),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_AICDA,color=ZCCHC7),size=1.5,width=0.12,alpha=1)+
...
ggsave(file="AICDA_DHL.pdf", plot=figure_2,bg = 'white', width =10, height = 10, units = 'cm', dpi = 600)
