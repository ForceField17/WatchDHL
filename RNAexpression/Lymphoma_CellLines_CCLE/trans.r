...
counts1 <- read.csv("./data/Final84_lymphoma_exp.txt", head=T, row.names = "gene",sep="\t")
nrow(counts1)
...
Sample_features <- read.delim("./data/Sample_all_CCLE_lymphoma.txt",sep = "\t",header = T)
...
selected <- which(samples$Info != "remove")
...
intgroup <- c("Type","Class", "Info","Label")
...
highlight <- d[which(d$Class!="Tumor"),]
myPCA <- ggplot()+theme_classic()
myPCA <- myPCA + geom_point(data = d, aes(x =PC1, y = PC2, shape = Type, fill = Label),alpha=0.6,size = 3.6) + 
...
myPCA <- ggplot()+theme_classic()
myPCA <- myPCA + geom_point(data = d, aes(x =PC1, y = PC2, fill = Label, shape = Type),alpha=0.6,size = 3.6) + 
...
F1a.plot<-F1a.plot+
  geom_point(data=Sample,aes(x=Zscore_ZCCHC7,y=Zscore_AICDA,shape=Type,color=Class,fill=Label),cex=1.8,alpha=.9)+
...
##################3333
Sample$xxx <- Sample$Type
Sample$xxx[which(Sample$Type!="DLBCL")] <- "other"
Sample$Type <- Sample$xxx 
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitterdodge(jitter.width = 0.2),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_AICDA,color=Type),size=1,width=0.15,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_AICDA),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
...
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(-1.1,2.5),breaks = seq(-10,10,1)) 
...
the<-compare_means(p.adjust.method = "none",Zscore_AICDA ~ Type,  data = Sample,paired = F )
...
ggsave(file="AICDA_XXX.pdf", plot=figure_2,bg = 'white', width =10, height = 12, units = 'cm', dpi = 600)


##################3333
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitterdodge(jitter.width = 0.2),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_ZCCHC7,color=Type),size=1,width=0.15,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_ZCCHC7),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
NAN_plot <-NAN_plot + theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(2,2,2,2),'lines'),
                            plot.title=element_text(size=34,vjust=0.5,hjust=0.5,face='bold.italic',color='transparent'),text=element_text(size=14,face='bold'),
                            legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.6,'cm'),legend.position='right',legend.text=element_text(size=14,hjust=0,face='bold'),
                            axis.text.x=element_text(size=12,face='bold',color='black'),axis.text.y=element_text(size=12,face='bold',color='black'),
                            axis.title.x=element_text(size=14,face='plain',color='black'),axis.title.y=element_text(size=14,face='plain',color='black'))
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(-2.3,3.9),breaks = seq(-10,10,1)) 
NAN_plot<- NAN_plot +ylab("Expression of ZCCHC7 (Z-score)") +xlab(NULL)
NAN_plot <- NAN_plot + scale_fill_manual(name=NULL,values = c("#bababa","transparent"))
NAN_plot <- NAN_plot + scale_color_manual(name=NULL,values = c("#ca0020","#f4a582","#b3de69"))
the<-compare_means(p.adjust.method = "none",Zscore_ZCCHC7 ~ Type,  data = Sample,paired = F )
#NAN_plot<- NAN_plot +stat_compare_means(comparisons = my_comparisons)
NAN_plot <- NAN_plot +stat_pvalue_manual(
  the, label = NULL, size = 3,linetype = 1,
  y.position = c( 8)
...
ggsave(file="ZCCHC7_XXX.pdf", plot=figure_2,bg = 'white', width =10, height = 12, units = 'cm', dpi = 600)


##################3333
NAN_plot <- ggplot() + theme_classic() 
NAN_plot <- NAN_plot + 
  geom_point(position=position_jitterdodge(jitter.width = 0.2),data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_PAX5,color=Type),size=1,width=0.15,alpha=1)+
  geom_boxplot(data=Sample,aes(x=reorder(Type,-as.integer(as.factor(Type))),y=Zscore_PAX5),width=0.4,alpha=0,size=0.4,outlier.size = 0.4,outlier.shape = NA)
NAN_plot <-NAN_plot + theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(2,2,2,2),'lines'),
                            plot.title=element_text(size=34,vjust=0.5,hjust=0.5,face='bold.italic',color='transparent'),text=element_text(size=14,face='bold'),
                            legend.key.width=unit(0.6,'cm'),legend.key.height=unit(0.6,'cm'),legend.position='right',legend.text=element_text(size=14,hjust=0,face='bold'),
                            axis.text.x=element_text(size=12,face='bold',color='black'),axis.text.y=element_text(size=12,face='bold',color='black'),
                            axis.title.x=element_text(size=14,face='plain',color='black'),axis.title.y=element_text(size=14,face='plain',color='black'))
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(-2.2,3.4),breaks = seq(-10,10,1)) 
NAN_plot<- NAN_plot +ylab("Expression of PAX5 (Z-score)") +xlab(NULL)
NAN_plot <- NAN_plot + scale_fill_manual(name=NULL,values = c("#bababa","transparent"))
NAN_plot <- NAN_plot + scale_color_manual(name=NULL,values = c("#ca0020","#f4a582","#b3de69"))
the<-compare_means(p.adjust.method = "none",Zscore_PAX5 ~ Type,  data = Sample,paired = F )
#NAN_plot<- NAN_plot +stat_compare_means(comparisons = my_comparisons)
NAN_plot <- NAN_plot +stat_pvalue_manual(
  the, label = NULL, size = 3,linetype = 1,
  y.position = c( 8)
...
ggsave(file="PAX5_XXX.pdf", plot=figure_2,bg = 'white', width =10, height = 12, units = 'cm', dpi = 600)
