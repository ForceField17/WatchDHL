...
while($line = <FILE>){
    ...
    ##FLa
        if($fileFLa eq "-"){
            ...
            $mapping1_2 =~ s/\n//g;
            $Nref_2[$i] = $mapping1_2;
            $mapping2_2 = `samtools view -b /scratch/PI/jgwang/dsongad/DLBCL/raw_data/results_T_DNA/$fileFLa\ $chr\:$pos\-$pos | samtools calmd -e - /scratch/PI/jgwang/dsongad/software/my_lib/star_genome_d1_vd1_gtfv22/GRCh38.d1.vd1.fa | grep -v "^@" | awk -F"\t" '(\$5>=30)' |  awk -v pos=$pos 'BEGIN {OFS = FS = "\t" } ; {n=split(\$10,a,"") ; if(a[(pos-\$4)+1] == "$ALT" ) print \$10 }' | sed s/=//g | awk '{print length}' | awk '(\$1<=5)' | wc -l`;
            $mapping2_2 =~ s/\n//g;
            $Nalt_2[$i] = $mapping2_2;
		}
    ##FLb
    ...
    ##FLa
        if($fileFLa eq "-"){
            ...
            $mapping1_2 =~ s/\n//g;
            $Nref_2[$i] = $mapping1_2;
            $mapping2_2 = `samtools view -b /scratch/PI/jgwang/dsongad/DLBCL/raw_data/results_T_DNA/$fileFLa\ $chr\:$pos\-$pos | samtools calmd -e - /scratch/PI/jgwang/dsongad/software/my_lib/star_genome_d1_vd1_gtfv22/GRCh38.d1.vd1.fa | grep -v "^@" | awk -F"\t" '(\$5>=30)' |  awk -v pos=$pos 'BEGIN {OFS = FS = "\t" } ; {theX=(pos-\$4+1); theY=$len; if(\$6 ~ theX"M"theY"D" ) print \$10 }' | sed s/=//g | awk '{print length}' | awk '(\$1<=5)' | wc -l`;
            $mapping2_2 =~ s/\n//g;
            $Nalt_2[$i] = $mapping2_2;
        }
    ##FLb
