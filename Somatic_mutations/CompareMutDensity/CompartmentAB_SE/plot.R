...
table <- read.table('Fraction.txt',header = T,sep = "\t")
...
revp_plot<-revp_plot+ theme(panel.background=element_rect(fill='transparent',color='transparent',size=1),plot.margin=unit(c(1,2,2,1),'lines'),
                            ...
                            legend.text=element_text(size=12,face='plain'),axis.text.y=element_text(size=12,angle=0,vjust=0.5,hjust=0.5,face='plain',color='black'),
...
revp_plot<-revp_plot+ggtitle(NULL)+xlab(NULL)+ylab("Fraction of SE")+scale_x_discrete(position = "bottom")+coord_flip()
...
revp_plot<-revp_plot+scale_fill_manual(name=NULL,values=c(BBB_nonSE='#d9d9d9',AAA_SE='#bc80bd'),labels=c(BBB_nonSE='nonSE',AAA_SE='SE'),guide=guide_legend(override.aes=list(size=0.5)),na.translate = F)
revp_plot
figure_2<-rbind(ggplotGrob(revp_plot),size="first")
ggsave(file="ExtFig5f_SEs_in_CompartmentAB.pdf", plot=figure_2,bg = 'white', width = 12, height = 6, units = 'cm', dpi = 600)


a <- data.frame(table$Num[1:2],table$Num[3:4])
#fisher.test(a)