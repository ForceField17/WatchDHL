...
plot.table <- read.table('final_results.txt',header=T)
plot.table <- plot.table[which(plot.table$case!="P4" & plot.table$case!="P5" & plot.table$compartment!="A_Random"),]

plot.table$fraction <- plot.table$nonWRCY/plot.table$mutation*100


plot.table2 <-plot.table
...
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(0,37),breaks = seq(0,100,5)) 
NAN_plot <- NAN_plot + scale_x_discrete(labels=c(B="B",A="A"))#
NAN_plot<- NAN_plot +ylab("percentage of nonWRCY mutation") +xlab(NULL)
...
ggsave(file="ExtFig16d_All_nonAID.pdf", plot=figure_2,bg = 'white', width =14, height = 10, units = 'cm', dpi = 600)

plot.table3 <-plot.table[which( plot.table$tumor=="FL"),] 
the<-compare_means(fraction ~ xxx,  data = plot.table3,paired = T,method = "wilcox.test" ,p.adjust.method = "fdr")
the

plot.table3 <-plot.table[which( plot.table$tumor=="DHL"),] 
the<-compare_means(fraction ~ xxx,  data = plot.table3,paired = T,method = "wilcox.test" ,p.adjust.method = "fdr")
the
