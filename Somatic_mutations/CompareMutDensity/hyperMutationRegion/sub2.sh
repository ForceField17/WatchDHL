...
bedtools window -a Input_peaks.bed -b AID/FL -w 0 | cut -f 4,8 | sort | uniq -c | awk '{print $3"+"$2"\t"$1}' | sort > $TMP\1
...
bedtools window -a Input_peaks.bed -b AID/DHL -w 0 | cut -f 4,8 | sort | uniq -c | awk '{print $3"+"$2"\t"$1}' | sort > $TMP\1
