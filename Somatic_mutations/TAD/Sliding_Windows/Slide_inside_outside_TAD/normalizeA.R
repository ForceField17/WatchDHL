...
cumAge <- cumAge+ggtitle(NULL)+xlab("Distance to TAD boundaries")+ylab('Average proportion of somatic mutations (%/Mbp)')+
  scale_y_continuous(expand=c(0,0),limits=c(0,0.62),breaks = seq(0,1,0.1)) +
...
figure_1<-cbind(ggplotGrob(cumAge),size="first")
ggsave(file="ExtFig5g_TAD_normalized_MB.pdf", plot=figure_1,bg = 'white', width = 17, height = 12, units = 'cm', dpi = 600)
