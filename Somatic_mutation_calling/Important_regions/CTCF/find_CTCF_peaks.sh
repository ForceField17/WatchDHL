cut -f 4 ../../../CellLineEpi/hg38.CTCF.sorted.bed  | sort > tmp
Total_FL=`tail -n+2 ../../step4.FL.csv | wc -l`
Total_DHL=`tail -n+2 ../../step4.DHL.csv | wc -l`
cut -f 121 ../../step4.DHL.csv | tail -n+2 | grep -v "\." | sort | uniq -c | awk -v xxx=$Total_DHL '{print $2"\t"($1/xxx)*100"\t"$1}' | sort > DHL_SE.txt
cut -f 121 ../../step4.FL.csv  | tail -n+2 | grep -v "\." | sort | uniq -c | awk -v xxx=$Total_FL '{print $2"\t"($1/xxx)*100"\t"$1}' | sort > FL_SE.txt
...
bedtools closest -a ../../../CellLineEpi/hg38.CTCF.sorted.bed  -b hg38.gene.bed -t first -d | awk '{print $4"\t"$3-$2"\t"$8}' | sort > List.txt
