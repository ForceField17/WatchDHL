cut -f 4 ../../../CellLineEpi/hg38.SE.sorted.bed  | sort > tmp
Total_FL=`tail -n+2 ../../step4.FL.csv | wc -l`
Total_DHL=`tail -n+2 ../../step4.DHL.csv | wc -l`
cut -f 129 ../../step4.DHL.csv | tail -n+2 | grep -v "\." | sort | uniq -c | awk -v xxx=$Total_DHL '{print $2"\t"($1/xxx)*100"\t"$1}' | sort > DHL_SE.txt
cut -f 129 ../../step4.FL.csv  | tail -n+2 | grep -v "\." | sort | uniq -c | awk -v xxx=$Total_FL '{print $2"\t"($1/xxx)*100"\t"$1}' | sort > FL_SE.txt
...
bedtools closest -a ../../../CellLineEpi/hg38.SE.sorted.bed  -b hg38.gene.bed -t first -d | awk '{print $4"\t"$3-$2"\t"$8}' | sort > List.txt
join tmp2 List.txt | sed s/" "/\\t/g | awk '{print $0"\t"$2*1000000/$6"\t"$4*1000000/$6}' | awk '{print $0"\t"$4-$2}' | sed s/CFAP251/BCL7A/g > MutationPercent.txt
