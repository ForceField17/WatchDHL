...
cumAge <- cumAge + geom_errorbar(data=All_Mut,width=0.5,size=0.5, aes(x=Distance,ymin=xxxx, ymax=Burden.avg+Burden.sd,color=V1),alpha=0.2)
...
cumAge <- cumAge+ggtitle(NULL)+xlab("H3K27ac")+ylab('Average proportion of somatic mutations (%/Mbp)')+
  scale_y_continuous(expand=c(0,0),limits=c(0,0.75),breaks = seq(0,1,0.1)) +
  scale_x_continuous(expand=c(0,0),limits=c(-7000,7000),breaks = seq(-200000,200000,2000))
...
cumAge2 <- cumAge2 + geom_errorbar(data=All_Mut,width=0.5,size=0.5, aes(x=Distance,ymin=xxxx, ymax=Burden.avg+Burden.sd,color=V1),alpha=0.2)
...
cumAge3 <- cumAge3 + geom_errorbar(data=All_Mut,width=0.5,size=0.5, aes(x=Distance,ymin=xxxx, ymax=Burden.avg+Burden.sd,color=V1),alpha=0.2)
...
ggsave(file="ZOOMIN_Slide_P126789_normalized_MB.pdf", plot=figure_1,bg = 'white', width = 34, height = 28, units = 'cm', dpi = 600)
