...
cat  ../result.txt | awk '{print $1"."$2"+"$3"\t"$4}' | sort > $TMP\2

join $TMP\1 $TMP\2 | sed s/" "/\\t/g | sed s/+/\\t/g | sort | awk -F"\t" -v A=$SIZE_A -v B=$SIZE_B '{if($2=="A"){print $0"\t"A}else{print $0"\t"B}}' | sed s/.FL/\\tFL/g | awk '{print "X_"$3"\t"$1"."$2"\t"$0}'
...
join $TMP\1 $TMP\2 | sed s/" "/\\t/g | sed s/+/\\t/g | sort | awk -F"\t" -v A=$SIZE_A -v B=$SIZE_B '{if($2=="A"){print $0"\t"A}else{print $0"\t"B}}' | sed s/.DHL/\\tDHL/g  | awk '{print "Y_"$3"\t"$1"."$2"\t"$0}'
