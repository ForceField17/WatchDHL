...
NAN_plot <- NAN_plot + scale_y_continuous(expand=c(0,0),limits=c(0,55),breaks = seq(0,100,10)) 
NAN_plot <- NAN_plot + scale_x_discrete(labels=c(B="B",A="A"))#
NAN_plot<- NAN_plot +ylab("percentage of WRCY mutation in SE") +xlab(NULL)
...
the<-compare_means(fraction ~ compartment,  data = plot.table2,paired = T,method = "wilcox.test" )
