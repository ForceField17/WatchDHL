setwd("/Users/songdong/Dropbox/Dropbox/DLBCL/main_figures/Mutation_Distribution/Pall_DistanceMutDensity/kata/Patients/")
...
Mut <- read.csv("./kateagis_DHL_P7.final.txt",header = T,sep = "\t")


MutHIGH <- Mut[which(Mut$histone != "zNone"),]
MutLOW <- Mut[which(Mut$histone == "zNone"),]
...
ffff_plot<-ffff_plot+#geom_point(data=MutHIGH ,aes(x=Position,y=log2(value)),size=1 ,color = "#b2182b")+
  geom_point(data=MutLOW ,aes(x=pos,y=(dis+1),color=histone),size=0.8 )+
  geom_point(data=MutHIGH ,aes(x=pos,y=(dis+1),color=histone),size=0.8)+
...
ffff_plot<-ffff_plot+scale_color_manual(name=NULL,values=c(H3K27="blue",H3K4="#0BF5E5",Both="#fc4e2a",zNone="grey"),
                                        labels=c(H3K27="H3K27ac",H3K4="H3K4me3",Both="Both",zNone="others"),
...
Mut_ <- read.csv("./kateagis_FL_P7.final.txt",header = T,sep = "\t")


Mut_HIGH <- Mut_[which(Mut_$histone!="zNone"),]
Mut_LOW <- Mut_[which(Mut_$histone=="zNone"),]
...
gggg_plot<-gggg_plot+#geom_point(data=MutHIGH ,aes(x=Position,y=log2(value)),size=1 ,color = "#b2182b")+
  geom_point(data=Mut_LOW ,aes(x=pos,y=(dis+1),color=histone),size=0.8 )+
  geom_point(data=Mut_HIGH ,aes(x=pos,y=(dis+1),color=histone),size=0.8)+
  #  geom_point(data=sig ,aes(x=start,y=log2(NumR_DHL),size=NumP_DHL) ,shape=21,stroke=1,color = "black")+
  #  geom_point(data=sigEnhan ,aes(x=start,y=log2(NumR_DHL),size=NumP_DHL) ,shape=21,fill = "blue",alpha=0.7)+
  #  geom_point(data=sigSE ,aes(x=start,y=log2(NumR_DHL),size=NumP_DHL) ,shape=21,,fill = "#d73027",alpha=0.8)+
  scale_y_continuous(expression(paste("Intermutation distance (bp)")),expand=c(0,0),limits = c(0.99, 9000000),breaks=c(1,100,10000,1000000,100000000),trans="log10")+
...
gggg_plot<-gggg_plot+scale_color_manual(name=NULL,values=c(H3K27="blue",H3K4="#0BF5E5",Both="#fc4e2a",zNone="grey"),
                                        labels=c(H3K27="H3K27ac",H3K4="H3K4me3",Both="Both",zNone="others"),
                                        guide = guide_legend(override.aes=list(size=3),nrow=1),na.translate = F)
#gggg_plot
...
ggsave(file="./merge.P7.HIS.pdf", plot=figure_1,bg = 'white', width = 34, height =  12, units = 'cm', dpi = 600)
...
ffff_plot<-ggplot()+theme_classic()
ffff_plot<-ffff_plot+#geom_point(data=MutHIGH ,aes(x=Position,y=log2(value)),size=1 ,color = "#b2182b")+
  geom_point(data=MutLOW ,aes(x=start/1000000,y=(dis+1),color=histone),size=0.8 )+
  geom_point(data=MutHIGH ,aes(x=start/1000000,y=(dis+1),color=histone),size=0.8)+
  #  geom_point(data=sigEnhan ,aes(x=start,y=log2(NumR_DHL),size=NumP_DHL) ,shape=21,fill = "blue",alpha=0.7)+
  #  geom_point(data=sigSE ,aes(x=start,y=log2(NumR_DHL),size=NumP_DHL) ,shape=21,,fill = "#d73027",alpha=0.8)+
  scale_y_continuous(expression(paste("Intermutation distance (bp)")),expand=c(0,0),limits = c(0.99, 9000000),breaks=c(1,100,10000,1000000,100000000),trans="log10")+
...
Mut <- read.csv("./kateagis_FL_P7.final.txt",header = T,sep = "\t")
...
figure_1$heights[panels][1] <- unit(1,'null')
figure_1$heights[panels][2] <- unit(1,'null')
ggsave(file="./merge.chr16.P7.HIS.pdf", plot=figure_1,bg = 'white', width = 34, height =  12, units = 'cm', dpi = 600)
