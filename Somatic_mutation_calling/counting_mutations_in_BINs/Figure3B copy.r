...
Mut$diffR <- Mut$NumP_DHL - Mut$NumP_FL
Mut$diffP <- abs(Mut$NumR_FL - Mut$NumR_DHL) + 1

MutLOW <- Mut[which(abs(Mut$diffR)<3),]
MutHIGH <- Mut[which(abs(Mut$diffR)>=3 ),]

sig <- Mut[which( (abs(Mut$diffR)>=3  & Mut$NumRecordsAll >= 5)),]

sigEnhan <- sig[which(grepl(pattern = 'H3K27',x = sig$Enhancer,fixed=T) ),]
sigSE <- sig[which(grepl(pattern = 'SE',x = sig$Enhancer,fixed=T) ),]
sigSE <- sigSE[order(sigSE$diffP,decreasing = F),]
...
ffff_plot<-ffff_plot+#geom_point(data=MutHIGH ,aes(x=Position,y=log2(value)),size=1 ,color = "#b2182b")+
  geom_point(data=MutLOW ,aes(x=as.numeric(as.character(start)),y=diffR),size=1 ,color = "grey")+
  #geom_point(data=MutHIGH ,aes(x=start,y=diffR),size=1 ,color = "black")+
  geom_point(data=sig ,aes(x=as.numeric(as.character(start)),y=diffR,size=diffP) ,shape=21,stroke=0.5,color = "black")+
  geom_point(data=sigEnhan ,aes(x=as.numeric(as.character(start)),y=diffR,size=diffP) ,shape=21,fill = "blue",alpha=0.8)+
  geom_point(data=sigSE ,aes(x=as.numeric(as.character(start)),y=diffR,size=diffP) ,shape=21,fill = "#d73027",alpha=0.8)+
  scale_y_continuous(expression(paste("log" ["2"], bold(" [mutation number]"))),expand=c(0,0),limits = c(-6, 6),breaks=seq(-8,8,2))+
...
ggsave(file="Figure3B_SE_all_points.pdf", plot=figure_1,bg = 'white', width = 27, height =  11.5, units = 'cm', dpi = 600)
