...
`sh grep1.sh >$target`;
open FILE, $target;
print "BIN_id\tchr\tstart\tend\tNumRecordsAll\tNumR_FL\tNumR_DHL\tNumPatientsAll\tNumP_FL\tNumP_DHL\tgene\tEnhancer\tHistone\n";
$i=0;
while($line = <FILE>){
	chomp($line);
	#$content[$i] = $line;
	...
	print "$H3\t$SE1\t$SE2\t$SE3\t$xx1\t$xx2\t$xx3\t$xx4\t$xx5\t$xx6\t$genes\t$hancer\t$histon\n";	
}
close FILE;
