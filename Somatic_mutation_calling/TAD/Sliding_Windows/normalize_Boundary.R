...
FL <-   read.delim("Slide_strong_TAD_boundary/mutDens_FL.txt",sep = c("\t"," "),header = F)
FL2 <-  read.delim("Slide_strong_TAD_boundary/mutDens_FL_nege.txt",sep = c("\t"," "),header = F)
DHL <-  read.delim("Slide_strong_TAD_boundary/mutDens_DHL.txt",sep = c("\t"," "),header = F)
DHL2 <- read.delim("Slide_strong_TAD_boundary/mutDens_DHL_nege.txt",sep = c("\t"," "),header = F)
...
cumAge <- cumAge+ggtitle(NULL)+xlab("Strong TAD boundary")+ylab('Average proportion of somatic mutations (%/Mbp)')+
...
#raw data preprocessing
FL <-   read.delim("Slide_weak_TAD_boundary/mutDens_FL.txt",sep = c("\t"," "),header = F)
FL2 <-  read.delim("Slide_weak_TAD_boundary/mutDens_FL_nege.txt",sep = c("\t"," "),header = F)
DHL <-  read.delim("Slide_weak_TAD_boundary/mutDens_DHL.txt",sep = c("\t"," "),header = F)
DHL2 <- read.delim("Slide_weak_TAD_boundary/mutDens_DHL_nege.txt",sep = c("\t"," "),header = F)
...
cumAge2<- cumAge2 +theme(panel.background=element_rect(fill='transparent',color='transparent'),plot.margin=unit(c(2,0.9,2,1),'lines'),
                         ...
                         axis.title.x=element_text(size=14,vjust=0,hjust=0.5,face="plain",color='black'),axis.title.y=element_blank())
cumAge2 <- cumAge2+ggtitle(NULL)+xlab("Weak TAD boundary")+ylab('Average proportion of somatic mutations (%/Mbp)')+
  scale_y_continuous(expand=c(0,0),limits=c(0,0.71),breaks = seq(0,1,0.1)) +
  scale_x_continuous(expand=c(0,0),limits=c(-100000,100000),breaks = seq(-200000,200000,40000))
table2 <- data.frame(c(0,0),c(0,0.0088))
colnames(table2) <- c("Xaxis","Yaxis")
...
figure_1<-cbind(ggplotGrob(cumAge),ggplotGrob(cumAge2),size="first")
ggsave(file="Slide_mergeA_normalized_MB.pdf", plot=figure_1,bg = 'white', width = 23, height = 14, units = 'cm', dpi = 600)
