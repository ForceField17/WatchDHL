...
for n in `cat list1`;do
	...
	cat Input_peaks.bed | awk -F"\t" -v DIS=$i '{if($4~/^-/){print $1"\t"$3-DIS"\t"$3}else{print $1"\t"$2"\t"$2+DIS}}' |  awk -F"\t" '{if($2<0){print $1"\t0\t"$3}else{print $0}}' |  awk -F"\t" '{if($3<0){print $1"\t"$2"\t1"}else{print $0}}' > $TMP\_old
