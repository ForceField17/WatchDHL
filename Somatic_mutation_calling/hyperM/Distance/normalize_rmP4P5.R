...
cumAge <- cumAge+ggtitle(NULL)+xlab("H3K27ac")+ylab('Average proportion of somatic mutations (%/Mbp)')+
  scale_y_continuous(expand=c(0,0),limits=c(0,2.5),breaks = seq(0,10,0.5)) +
...
cumAge2 <- cumAge2+ggtitle(NULL)+xlab("H3K4me3")+ylab('Average proportion of somatic mutations (%/Mbp)')+
  scale_y_continuous(expand=c(0,0),limits=c(0,2.5),breaks = seq(0,4,0.5)) +
  scale_x_continuous(expand=c(0,0),limits=c(-100000,100000),breaks = seq(-200000,200000,40000))
table2 <- data.frame(c(0,0),c(0,0.0088))
colnames(table2) <- c("Xaxis","Yaxis")
...
figure_1<-cbind(ggplotGrob(cumAge4),ggplotGrob(cumAge5),size="first")
ggsave(file="Slide_xx_MB.pdf", plot=figure_1,bg = 'white', width = 22, height = 14, units = 'cm', dpi = 600)
